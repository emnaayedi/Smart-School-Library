...
console.log(msg);
        ...
        rootRef.orderByChild('emp_id').equalTo(msg).on("child_added",snap => {
         ...
         var ref_livre=firebase.database().ref('emprunte/' + msg).child("id_livre");
          ...
          ref_livre.on('value', function(snap_id) {
              var idel = snap_id.val();
              idl=idel;
          });
                        console.log(idl);

 etudiant.orderByChild('login').equalTo(ide).on('child_added', function(snap) {
  var reff=firebase.database().ref('etudiants/' + ide).child("nb_emp");
          var b;        var a;
 ...
 stockage.orderByKey().equalTo(idl).on('child_added', function(snap) {

      var ref=firebase.database().ref('stockage/' + idl).child("Qte");
